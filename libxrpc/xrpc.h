...
/**
 *  XR_DTSTAT -- Struct needed for the status monitor.
 */
typedef struct {
    long NumCallSync;
    long NumMethod;
...
/*  xrHandles.c
*/
void      xr_initHandles (void);
void      xr_cleanupHandles (void);
handle_t  xr_setHandle (int handle, void *ptr);
handle_t  xr_newHandle (void *ptr);
void      xr_freeHandle (handle_t handle);
handle_t  xr_P2H (void *ptr);
void     *xr_H2P (handle_t handle);

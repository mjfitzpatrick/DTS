...
#define	 DEBUG			0
#define  SRVR_DEBUG  (getenv("SRVR_DBG")!=NULL||access("/tmp/SRVR_DBG",F_OK)==0)
...
*/
#define SINGLE_RUN_SERVER	1		// mostly works

#define SZ_CALL_RING		1024
...
xr_createServer (char *path, int port, char *logfile)
{
    ...
    */
    ...
    sprintf (svr->url, "http://127.0.0.1:%d/RPC2", port);
...
xr_startServerThread ()
{
    ...
    */
    stat = pthread_create (&svr->thread, NULL, (void *)xr_rpcListener,
		(void *) svr);
    if (stat != 0) {
               ...
               xmlrpc_value *paramArrayP, void *serverInfo)
{
    ...
    extern   int  xr_errstat, xmlrpc_refcount();
    ...
    */
	...
	if (strcmp (m->name, methodName) == 0) {
	    ...
	    /* Call the function.
	    xr_setStat (2);		// FIXME - not thread safe
	    */
...
/*
		...
		if (xmlrpc_refcount(paramArrayP) == 0)
		    paramArrayP = (xmlrpc_value *) NULL;
*/
	        result = ((c && c->result) ? c->result : (xmlrpc_value *) NULL);
	    }

	    if (result) xmlrpc_INCREF(result);
...
#ifdef HAVE_ON_EXIT
void xr_atexit (int status, void*data)
{
    if (status)
        abort ();    /* dump a diagnostc core file, we should never get here */
}
#else
void xr_atexit (void)
{
...
xr_rpcListener (Server *svr)
{
	...
	*/
    ...
    /* Dump a core file if we exit.
     */
    if (access ("/tmp/SRVR_DEBUG", R_OK) == 0) {
	fprintf (stderr, "xr_rpcListener: running server ...pid=%d  tid=%d\n", 
   		(int)getpid(), (int)syscall(SYS_gettid));
#ifdef HAVE_ON_EXIT
	on_exit (xr_atexit, NULL);
#else
	atexit (xr_atexit);
#endif
    }

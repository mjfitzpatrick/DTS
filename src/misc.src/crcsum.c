...
unsigned int dts_fileCRCChecksum (char *fname, unsigned int *crc_out);
unsigned int dts_fileChecksum (char *fname, int do_sysv);
unsigned int dts_fileCRC32 (char *fname);
...
main(int argc, char *argv[])
{
    ...
    sum32 = dts_fileChecksum (argv[1], 0);
    fprintf (stderr, "BSD Checksum: sum = %d\n", sum32);

    sum32 = dts_fileChecksum (argv[1], 1);
    fprintf (stderr, "SysV Checksum: sum = %d\n", sum32);

    crc = dts_fileCRC32 (argv[1]);
    fprintf (stderr, "CRC-32: crc = %d\n", crc);
...
static unsigned int crc_tab[] = {
    ...
    0xc30c8ea1, 0x5a05df1b, 0x2d02ef8d, 0x0
};
...
dts_fileCRCChecksum (char *fname, unsigned int *crc_out)
{
	 ...
	 */
        for (i=0; i < bytes_read; i++) {
            crc = ((crc >> 8) & 0x00FFFFFF) ^ crc_tab[(crc ^ buf[i]) & 0xFF];
            s += buf[i];
	}
    }

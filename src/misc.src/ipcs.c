...
static void
help (void) {
	printf ("%s provides information on ipc facilities for \
...
void do_shm (char format)
{
	...
	case LIMITS:
		...
		printf ("max number of segments = %ld\n",
			(long) shminfo.shmmni);
		printf ("max seg size (kbytes) = %ld\n",
			(long) (shminfo.shmmax >> 10));
		printf ("max total shared memory (kbytes) = %ld\n",
			(long) shminfo.shmall << 2);
		printf ("min seg size (bytes) = %ld\n",
			(long) shminfo.shmmin);
		return;
...
void do_sem (char format)
{
	...
	default:
		...
		printf ("%-10s %-10s %-10s %-10s %-10s %-10s\n", 
			"key","semid","owner","perms", "nsems", "value");
		break;
	}
...
void do_msg (char format)
{
	...
	if (maxid < 0) {
		printf ("kernel not configured for message queues\n");
		return;
	}
...
void print_shm (int shmid)
{
	...
	printf ("bytes=%ld\tlpid=%d\tcpid=%d\tnattch=%ld\n",
